...
climate_spiral <- function(data = NULL, by_year = FALSE, animate = FALSE){
  ...
  gg_dat <- dplyr::bind_rows(temp, next_jan) %>%
    dplyr::mutate(month = factor(month, levels = c(month.abb, "next_Jan")),
  ...
  if (isTRUE(by_year)){
    ...
    gg <- gg_dat %>%
      ...
      ggplot2::geom_col(data = month_labels, ggplot2::aes(x=x + 0.5, y=2.4), fill = "black",
                        width  = 1,
                        inherit.aes = FALSE) +
      ggplot2::geom_col(data = month_labels, ggplot2::aes(x=x + 0.5, y=-2), fill = "black",
                        width  = 1,
                        inherit.aes = FALSE) +
      ggplot2::geom_hline(yintercept = c(1.5, 2.0), color="red") +
      ggplot2::geom_line() +
      ggplot2::geom_point(data=annotation, ggplot2::aes(x=month_number, y=t_diff, color=Year),
                          size = 2,
                          inherit.aes = FALSE) +
      ggplot2::geom_label(data = temp_lines, ggplot2::aes(x=x, y=y, label=labels),
                          color = "red", fill = "black", label.size = 0,
                          inherit.aes=FALSE) +
      ggplot2::geom_text(data = month_labels, ggplot2::aes(x=x, y=y, label = labels),
      ...
      ggplot2::scale_y_continuous(breaks = seq(-2, 2, 0.2),
                                  limits = c(-2, 2.7), expand = c(0, -0.7),
                                  sec.axis = ggplot2::dup_axis(name = NULL, labels=NULL)) +
      ggplot2::scale_color_viridis_c(breaks = seq(1880, 2020, 20),
                                     guide = "none") +
  ...
  if (isFALSE(by_year)){
    ...
    gg <- gg_dat %>%
      ...
      ggplot2::geom_label(ggplot2::aes(x = 1, y=-1.7, label = Year),
                          ...
                          size=6) +
      ggplot2::geom_line() +
      ggplot2::geom_segment(data=gridlines, ggplot2::aes(x=x, y=y, xend=xend, yend=yend),
                            color=c("yellow", "green", "yellow"),
                            inherit.aes = FALSE) +
      ggplot2::geom_text(data = temp_lines, ggplot2::aes(x=x, y=y, label=labels),
      ...
      ggplot2::scale_color_gradient2(low = "blue", high = "red", mid="white", midpoint = 0,
                                     guide = "none") +
  ...
  if (isTRUE(animate)){
    ...
    gganimate::animate(gg, width=4.155, height=4.5, unit="in", res=300,
