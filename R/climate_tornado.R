...
climate_tornado <- function(data = NULL){
  ...
  gg_dat %>%
    ...
    ggplot2::geom_label(data = grid_labels, ggplot2::aes(x=x, y=y, label=labels),
               inherit.aes = FALSE,
               fill = "black", color="gold", label.size = 0, size = 3) +
    ggplot2::geom_segment(size=0.9, lineend = "round") +
    ggplot2::geom_text(data = year_labels, aes(x=x, y=y, label=y),
              inherit.aes = FALSE, color = "gold", size= 3, fontface = "bold") +
    ggplot2::scale_color_gradient2(low = "darkblue", high ="darkred", mid ="white",
